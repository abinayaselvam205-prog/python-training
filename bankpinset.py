...
BankAccount(name.strip(), pin, initial_balance) 
            ...
            st.success(f"Account created for {name} with 
₹{initial_balance}") 
            st.info("Go to 'Login' page from the sidebar to 
access ATM services.") 
...
{st.session_state.account.account_holder}! Login 
successful.") 
                st.info("Go to 'ATM Operations' page to perform 
transactions.") 
            else: 
...
min_value=0, step=100, key="dep") 
            ...
            amount = st.number_input("Enter withdrawal 
amount", min_value=0, step=100, key="with") 
            if st.button("Withdraw"): 
                st.info(st.session_state.account.withdraw(amoun
 t)) 
 
        elif action == "Check Balance": 
            st.success(f"Your current balance is 
₹{st.session_state.account.get_balance()}") 
